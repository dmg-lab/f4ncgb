...
template<typename T>
inline void
sparse_mat_clear_zero_row(sparse_mat_t<T> mat) {
  ulong new_nrow = 0;
  for(size_t i = 0; i < mat->nrow; i++) {
    if(mat->rows[i].nnz != 0) {
...
template<typename T>
inline void
sparse_mat_transpose(sparse_mat_t<T> mat2, const sparse_mat_t<T> mat) {
...
template<typename T>
inline void
sparse_mat_transpose(sparse_mat_t<bool> mat2, const sparse_mat_t<T> mat) {
  for(size_t i = 0; i < mat2->nrow; i++)
    sparse_mat_row(mat2, i)->nnz = 0;
...
template<typename T>
inline void
sparse_mat_transpose_part(sparse_mat_t<T> mat2,
                          const sparse_mat_t<T> mat,
                          const std::vector<slong>& rows) {
  for(size_t i = 0; i < mat2->nrow; i++)
    sparse_mat_row(mat2, i)->nnz = 0;
  ...
  for(size_t i = 0; i < rows.size(); i++) {
    ...
    for(size_t j = 0; j < therow->nnz; j++) {
      auto col = therow->indices[j];
      _sparse_vec_set_entry(
        sparse_mat_row(mat2, col), row, therow->entries + j);
    }
  }
}
...
template<typename T>
inline void
sparse_mat_transpose_part(sparse_mat_t<bool> mat2,
                          const sparse_mat_t<T> mat,
                          const std::vector<slong>& rows) {
  for(size_t i = 0; i < mat2->nrow; i++)
    sparse_mat_row(mat2, i)->nnz = 0;
...
template<typename T>
inline int
sparse_mat_dot_sparse_vec(sparse_vec_t<T> result,
                          const sparse_mat_t<T> mat,
                          const sparse_vec_t<T> vec,
                          field_t F) {
...
template<typename T>
inline int
sparse_mat_dot_sparse_mat(sparse_mat_t<T> A,
                          sparse_mat_t<T> B,
                          sparse_mat_t<T> C,
                          field_t F) {
  if(B->ncol != C->nrow)
  ...
  for(size_t i = 0; i < B->nrow; i++)
    sparse_mat_dot_sparse_vec(
      sparse_mat_row(A, i), B, sparse_mat_row(Ct, i), F);
...
// 	sparse_mat_t<T*> tranmat, std::vector<slong>& donelist, bool is_tran =
// false) { 	auto localcounter = 0; 	std::vector<slong> pivlist(mat->nrow, -1);
...
auto
findmanypivots_r(sparse_mat_t<T> mat,
                 const sparse_mat_t<S> tranmat,
                 std::vector<slong>& colpivs,
                 std::vector<slong>& rowperm,
...
template<typename T, typename S>
auto
findmanypivots_c(sparse_mat_t<T> mat,
                 sparse_mat_t<S> tranmat,
                 std::vector<slong>& rowpivs,
                 std::vector<slong>& colperm,
                 std::vector<slong>::iterator start,
                 size_t max_depth = ULLONG_MAX) {
  ...
  for(size_t i = 0; i < mat->nrow; i++) {
    ...
    for(size_t j = 0; j < tc->nnz; j++) {
      ...
      // make the result stable
      else if(tranmat->rows[tc->indices[j]].nnz == mnnz
              && tc->indices[j] < col) {
        col = tc->indices[j];
      }
    }
...
template<typename T>
void
triangular_solver(sparse_mat_t<T> mat,
...
template<typename T>
void
schur_complete(sparse_mat_t<T> mat,
               slong row,
               std::vector<std::pair<slong, slong>>& pivots,
               int ordering,
               field_t F,
               ...
               bool mode) {
  if(ordering < 0) {
    std::vector<std::pair<slong, slong>> npivots(pivots.rbegin(),
                                                 pivots.rend());
  ...
  auto rrefonerow = [&](auto& nonzero_c) {
    ...
    for(auto [r, c] : pivots) {
      ...
      for(size_t i = 0; i < row->nnz; i++) {
        ...
        if constexpr(std::is_same_v<T, ulong>) {
          tmpvec[row->indices[i]] = _nmod_sub(
            tmpvec[row->indices[i]],
            n_mulmod_shoup(*entry, row->entries[i], e_pr, F->pvec[0].n),
            F->pvec[0]);
...
sparse_mat_rref_c(sparse_mat_t<T> mat,
                  field_t F,
                  BS::thread_pool& pool,
                  rref_option_t opt) {
  ...
  std::stable_sort(
    colperm.begin(), colperm.end(), [&tranmatp](slong a, slong b) {
  ...
  while(kk < mat->ncol) {
    ...
    auto ps = findmanypivots_c(
      mat, tranmat, rowpivs, colperm, colperm.begin() + kk, opt->search_depth);
    if(ps.size() == 0)
      break;
    ...
    bool mode
      = ((double)100 * now_nnz / (leftrows.size() * mat->ncol) < SPARSE_BOUND);
    pool.detach_blocks<ulong>(
      0,
      leftrows.size(),
      [&](const ulong s, const ulong e) {
    ...
    while(localcount < leftrows.size()) {
      for(size_t i = 0; i < leftrows.size(); i++) {
        if(flags[i]) {
          ...
          for(size_t j = 0; j < therow->nnz; j++) {
            auto col = therow->indices[j];
            _sparse_vec_set_entry(
              sparse_mat_row(tranmat, col), row, (bool*)NULL);
          }
          flags[i] = 0;
          localcount++;
        }
      ...
      if(verbose && (print_once || pr - oldpr > opt->print_step)) {
        ...
        std::cout << "-- Col: " << std::setw(bitlen_ncol) << (int)pr << "/"
                  << mat->ncol << "  rank: " << std::setw(bitlen_ncol)
                  << pivots.size() << "  nnz: " << std::setw(bitlen_nnz)
                  << now_nnz << "  density: " << std::setprecision(6)
                  << std::setw(8)
...
template<typename T>
std::vector<std::pair<slong, slong>>
sparse_mat_rref_r(sparse_mat_t<T> mat,
                  field_t F,
                  BS::thread_pool& pool,
                  rref_option_t opt) {
  // first canonicalize, sort and compress the matrix
  ...
  // sort rows by nnz
  std::stable_sort(rowperm.begin(), rowperm.end(), [&mat](slong a, slong b) {
  ...
  while(kk < mat->nrow) {
    ...
    pool.wait();
    auto ps = findmanypivots_r(
      mat, tranmat, colpivs, rowperm, rowperm.begin() + kk, opt->search_depth);
    ...
    for(auto i = rowperm.begin() + kk; i != rowperm.end(); i++)
      now_nnz += mat->rows[*i].nnz;
    bool mode
      = ((double)100 * now_nnz / ((mat->nrow - kk) * mat->ncol) < SPARSE_BOUND);
    pool.detach_blocks<ulong>(
      kk,
      mat->nrow,
      [&](const ulong s, const ulong e) {
    ...
    for(size_t i = 0; i < tranmat->nrow; i++)
      tranmat->rows[i].nnz = 0;
    // compute the transpose of the submatrix and print the status
    // asynchronizely
    while(tran_count < leftrows.size()) {
      ...
      auto status
        = (kk - newpiv + 1) + ((double)tran_count / (mat->nrow - kk)) * newpiv;
...
template<typename T>
std::vector<std::pair<slong, slong>>
sparse_mat_rref(sparse_mat_t<T> mat,
                field_t F,
                BS::thread_pool& pool,
                rref_option_t opt) {
  std::vector<std::pair<slong, slong>> pivots;
...
// { 	auto ratio = sizeof(ulong) / sizeof(char); 	auto nnz =
// sparse_mat_nnz(mat); 	auto len = (3 + mat->nrow + 2 * nnz) * ratio; 	char*
// buffer = s_malloc<char>(len); 	char* ptr = buffer; 	std::memcpy(ptr,
// &(mat->nrow), sizeof(ulong)); ptr += ratio; 	std::memcpy(ptr, &(mat->ncol),
// sizeof(ulong)); ptr += ratio; 	std::memcpy(ptr, &nnz, sizeof(ulong)); ptr +=
// ratio; 	for (size_t i = 0; i < mat->nrow; i++) { 		auto therow =
// sparse_mat_row(mat, i); 		std::memcpy(ptr, &(therow->nnz), sizeof(ulong)); ptr
// += ratio; 		std::memcpy(ptr, therow->indices, therow->nnz * sizeof(ulong)); ptr
// += therow->nnz * ratio; 		std::memcpy(ptr, therow->entries, therow->nnz *
// sizeof(ulong)); ptr += therow->nnz * ratio;
// 	}
...
// 		std::memcpy(therow->indices, ptr, therow->nnz * sizeof(ulong)); ptr +=
// therow->nnz * ratio; 		std::memcpy(therow->entries, ptr, therow->nnz *
// sizeof(ulong)); ptr += therow->nnz * ratio;
// 	}
// }
