...
#define sparse_vec_entry_pointer(vec, index) ((vec)->entries + (index))
  ...
  template<typename T>
  inline void sparse_vec_init(
    sparse_vec_t<T> vec, ulong alloc = 1, ulong rank = 1) {
    vec->nnz = 0;
    vec->alloc = alloc;
...
#define sparse_vec_zero(__vec) ((__vec)->nnz = 0)
  ...
  template<typename T>
  inline void sparse_vec_clear(sparse_vec_t<T> vec) {
    vec->nnz = 0;
    vec->alloc = 0;
    s_free(vec->indices);
    vec->indices = NULL;
  ...
  template<typename T>
  inline T* sparse_vec_entry(
    sparse_vec_t<T> vec, slong index, const bool isbinary = true) {
    ...
    if constexpr(std::is_same_v<T, bool>) {
      ...
      if constexpr(is_scalar_s<T>::value) {
        ...
        for(size_t i = 0; i < vec->nnz; i++) {
          scalar_set(entries + i, sparse_vec_entry_pointer(vec, perm[i]));
        }
  ...
  static inline void sparse_vec_rescale(
    snmod_vec_t vec, const ulong* scalar, const field_t F) {
    _nmod_vec_scalar_mul_nmod_shoup(
      vec->entries, vec->entries, vec->nnz, *scalar, *(F->pvec));
  }
  ...
  // s_malloc<char>((1
  // + 2 * vec->nnz) * ratio); 	std::memcpy(buffer, &(vec->nnz), sizeof(ulong));
  // 	std::memcpy(buffer + ratio, vec->indices, vec->nnz * sizeof(ulong));
  // 	std::memcpy(buffer + (1 + vec->nnz) * ratio, vec->entries, vec->nnz *
  // sizeof(ulong)); 	return std::make_pair((1 + 2 * vec->nnz) * ratio, buffer);
  // }

  // static void snmod_vec_from_binary(sparse_vec_t<ulong> vec, const char*
  // buffer) { 	auto ratio = sizeof(ulong) / sizeof(char);
  // std::memcpy(&(vec->nnz), buffer, sizeof(ulong)); 	sparse_vec_realloc(vec,
  // vec->nnz); 	std::memcpy(vec->indices, buffer + ratio, vec->nnz *
  // sizeof(ulong)); 	std::memcpy(vec->entries, buffer + (1 + vec->nnz) * ratio,
  // vec->nnz * sizeof(ulong));
  // }

  // debug only, not used to the large vector
  template<typename T>
  void print_vec_info(const sparse_vec_t<T> vec) {
    std::cout << "-------------------" << std::endl;
